/// \file 
/// \brief Definitions related to the Positive_Product_Binary_Constraint class

#ifndef POSITIVE_PRODUCT_BINARY_CONSTRAINT_H
#define POSITIVE_PRODUCT_BINARY_CONSTRAINT_H

#include "binary_constraint.h"


class Positive_Product_Binary_Constraint: public Binary_Constraint   /// This class implements the binary constraint imposing  x.y >= 0  \ingroup core
{
	public:
		// constructors and destructor
		Positive_Product_Binary_Constraint (vector<Variable *> & var);									      ///< constructs a new constraint which involves the variable in var and whose relation imposes that the product of the two numbers is positive or zero
		Positive_Product_Binary_Constraint (Positive_Product_Binary_Constraint & c);	///< constructs a new constraint by copying the constraint c 
...
inline Constraint * Positive_Product_Binary_Constraint::Duplicate ()
// returns a pointer on a copy of the constraint
{
	return new Positive_Product_Binary_Constraint (*this);
}


inline bool Positive_Product_Binary_Constraint::Impacted_By_Last_Events (CSP * pb, timestamp ref)
