...
Greater_Weighted_Sum_Global_Constraint::Greater_Weighted_Sum_Global_Constraint (Greater_Weighted_Sum_Global_Constraint & c): Weighted_Sum_Global_Constraint (c)
...
// returns true if the application of arc-consistency on the constraint w.r.t. the variable var deletes a value in the domain of var, false otherwise
{
	...
	for (unsigned int i = 0; i < arity; i++)
		if (scope_var[i]->Get_Num() != var)
		{
			...
			if (weights[i] > 0)
				sum += dy->Get_Real_Max() * weights[i];
			else sum += dy->Get_Real_Min() * weights[i];
		}
  ...
  int x = Get_Position (var);
	...
	if (weights[x] > 0)
	{
		if (sum+dx->Get_Real_Min()*weights[x] < constant)
		{
			int i = dx->Get_Min();
			modif = true;
			...
			while ((i != -1) && (sum + dx->Get_Real_Value (i) * weights[x] < constant));			
		}
	}
	else
		if (sum+dx->Get_Real_Max()*weights[x] < constant)
		{
			int i = dx->Get_Max();
			modif = true;
			ds->Add_Element (scope_var[x]);
