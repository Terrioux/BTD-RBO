/// \file 
/// \brief Definitions related to the All_Different_Except_Global_Constraint class

#ifndef ALL_DIFFERENT_EXCEPT_GLOBAL_CONSTRAINT_H
#define ALL_DIFFERENT_EXCEPT_GLOBAL_CONSTRAINT_H

#include "global_constraint.h"

class All_Different_Except_Global_Constraint: public Global_Constraint      /// This class implements the all-diff global constraint with an exception value \ingroup core
{
  protected:
    int exception_value;    ///< the value which is not taken into account in the all-different constraint
	...
	public:
		...
		All_Different_Except_Global_Constraint (vector<Variable *> & var, int exception);      ///< constructs a new all-diff global constraint with an exception value
		All_Different_Except_Global_Constraint (All_Different_Except_Global_Constraint & c);	 ///< constructs a new constraint by copying the constraint c
		~All_Different_Except_Global_Constraint ();														         ///< destructor
...
inline Constraint * All_Different_Except_Global_Constraint::Duplicate ()
// returns a pointer on a copy of the constraint
{
	return new All_Different_Except_Global_Constraint (*this);
}


inline bool All_Different_Except_Global_Constraint::Impacted_By_Last_Events (CSP * pb, timestamp ref)
