/// \file 
/// \brief Definitions related to the Length_K_Dim_No_Overlap_Global_Constraint class

#ifndef LENGTH_K_DIM_NO_OVERLAP_GLOBAL_CONSTRAINT_H
#define LENGTH_K_DIM_NO_OVERLAP_GLOBAL_CONSTRAINT_H

#include "global_constraint.h"


class Length_K_Dim_No_Overlap_Global_Constraint: public Global_Constraint   /// This class implements the No-Overlap global constraint for K dimension \ingroup core
{
	protected:
	...
	public:
		...
		Length_K_Dim_No_Overlap_Global_Constraint (vector<Variable *> & var, unsigned int nb_dim);		///< constructs a new constraint which involves the variable in var
		Length_K_Dim_No_Overlap_Global_Constraint (Length_K_Dim_No_Overlap_Global_Constraint & c);		///< constructs a new constraint by copying the constraint c
...
inline Constraint * Length_K_Dim_No_Overlap_Global_Constraint::Duplicate ()
// returns a pointer on a copy of the constraint
{
	return new Length_K_Dim_No_Overlap_Global_Constraint (*this);
}


inline bool Length_K_Dim_No_Overlap_Global_Constraint::Impacted_By_Last_Events (CSP * pb, timestamp ref)
