...
Negative_Product_Binary_Constraint::Negative_Product_Binary_Constraint (vector<Variable *> & var): Binary_Constraint (var)
// constructs a new constraint which involves the variable in var and whose relation imposes that the product of the two numbers is negative or zero
{
}


Negative_Product_Binary_Constraint::Negative_Product_Binary_Constraint (Negative_Product_Binary_Constraint & c): Binary_Constraint (c)
...
bool Negative_Product_Binary_Constraint::Is_Satisfied (int * t)
...
bool Negative_Product_Binary_Constraint::Revise (CSP * pb, unsigned int var, Support * ls, Deletion_Stack * ds)
...
void Negative_Product_Binary_Constraint::Propagate (CSP * pb, Assignment & A, Support * ls, Deletion_Stack * ds, timestamp ref)
...
string Negative_Product_Binary_Constraint::Get_XCSP3_Expression()
// returns the string corresponding to the expression of the constraint in XCSP 3 format
{
  return "<intension> le(mul(" + scope_var[0]->Get_Name() + "," + scope_var[1]->Get_Name() + "),0) </intension>";
}
