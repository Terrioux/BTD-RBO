...
Maximum_Objective_Function::Maximum_Objective_Function (Objective obj, vector<Variable *> obj_scope): Objective_Function(obj, obj_scope)
// constructs an objective function involving the objective obj and a maximum based on the variables of obj_scope
{
  criterion_information = "maximum "+to_string(scope.size());
  x_obj = 0;
}
...
void Maximum_Objective_Function::Update_Problem (COP * pb)
// updates the problem by adding some variables or constraints to the problem to handle the objective function
{
  if (! problem_updated)
  {
    ...
    pb->Add_Constraint (new Maximum_Global_Constraint (scope_ctr),false);
...
long Maximum_Objective_Function::Get_Cost (Assignment & A)
...
pair<long,long> Maximum_Objective_Function::Get_Cost_Interval ()
