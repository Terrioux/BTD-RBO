...
Maximum_Global_Constraint::Maximum_Global_Constraint (vector<Variable *> & var): Global_Constraint (var)
// constructs a maximum global constraint 
{
}
		
		
Maximum_Global_Constraint::Maximum_Global_Constraint (Maximum_Global_Constraint & c): Global_Constraint (c)
...
bool Maximum_Global_Constraint::Is_Satisfied (int * t)
// returns true if the tuple t satisfies the constraint, false otherwise
{
  int max = scope_var[1]->Get_Domain()->Get_Real_Value(t[1]);
  ...
  return max == scope_var[0]->Get_Domain()->Get_Real_Value(t[0]);
}


bool Maximum_Global_Constraint::Revise (CSP * pb, unsigned int var, Support * ls, Deletion_Stack * ds)
// returns true if the application of arc-consistency on the constraint w.r.t. the variable var deletes a value in the domain of var, false otherwise
{
  ...
  if (x == 0)
  {
    ...
    for (unsigned int i = 1; i < arity; i++)
      if ((scope_var[i]->Get_Domain()->Get_Size() == 1) && (max < scope_var[i]->Get_Domain()->Get_Remaining_Real_Value(0)))
        max = scope_var[i]->Get_Domain()->Get_Remaining_Real_Value(0);
    ...
    {
      // we remove the values smaller than the maximum of the reference variable x0
      Domain * dy;
      
      dy = scope_var[0]->Get_Domain();
      int max = dy->Get_Real_Max();
  
      unsigned int current_size = dx->Get_Size();
      ds->Add_Element (scope_var[x]);
      dx->Filter_Value_From (max+1);
      ...
      // we check whether the values greater than the maximum of the other list variables has a support w.r.t. x0
      max = INT_MIN;
      unsigned int i = 1;
      ...
      while ((v != -1) && ((real_v = dx->Get_Real_Value (v)) >= max))
      {
...
void Maximum_Global_Constraint::Propagate (CSP * pb, Assignment & A, Support * ls, Deletion_Stack * ds, timestamp ref)
// applies the event-based propagator of the constraint by considering the events occurred since ref
{
  ...
  do
  {
    ...
    {
      // we look for useless values as soon as the maximum value of the first variable has been modified
      int real_max = dx->Get_Real_Max();
      Domain * dy;
      ...
      for (unsigned int i = 1; i < arity; i++)
      {
        ...
        dy->Filter_Value_From (real_max+1);
    ...
    for (unsigned int i = 1; i < arity; i++)
    {
      ...
      if (event_manager->Exist_Event_Ubc (num_var, ref))
      {
        if (todo)
        {
          int max = scope_var[1]->Get_Domain()->Get_Real_Max();
          for (unsigned int j = 2; j < arity; j++)
            if (scope_var[j]->Get_Domain()->Get_Real_Max() > max)
              max = scope_var[j]->Get_Domain()->Get_Real_Max();
        ...
        {
          // we remove all the values of the first variable which are smaller than the value assigned to the ith variable
          int real_val = scope_var[i]->Get_Domain()->Get_Remaining_Real_Value(0);

          ds->Add_Element (scope_var[0]);
          dx->Filter_Value_Before(real_val-1);
...
string Maximum_Global_Constraint::Get_XCSP3_Expression()
