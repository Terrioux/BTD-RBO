/// \file 
/// \brief Definitions related to the Lex_Global_Constraint class

#ifndef LEX_GLOBAL_CONSTRAINT_H
#define LEX_GLOBAL_CONSTRAINT_H

#include "global_constraint.h"

class Lex_Global_Constraint: public Global_Constraint      /// This class implements the lex global constraint \ingroup core
{
  protected:
    bool or_equal;                 ///< true if the relation is not a strict inequality, false otherwise
    unsigned int tuple_size;       ///< the size of each tuple
  
	public:
		// constructors and destructor
		Lex_Global_Constraint (vector<Variable *> & var, bool equal);  ///< constructs a new lex global constraint
		Lex_Global_Constraint (Lex_Global_Constraint & c);	           ///< constructs a new constraint by copying the constraint c
...
inline Constraint * Lex_Global_Constraint::Duplicate ()
// returns a pointer on a copy of the constraint
{
	return new Lex_Global_Constraint (*this);
}


inline bool Lex_Global_Constraint::Impacted_By_Last_Events (CSP * pb, timestamp ref)
