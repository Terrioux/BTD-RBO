...
Not_Equal_Sum_Global_Constraint::Not_Equal_Sum_Global_Constraint (Not_Equal_Sum_Global_Constraint & c): Sum_Global_Constraint (c)
...
bool Not_Equal_Sum_Global_Constraint::Revise (CSP * pb, unsigned int var, Support * ls, Deletion_Stack * ds)
// returns true if the application of arc-consistency on the constraint w.r.t. the variable var deletes a value in the domain of var, false otherwise
{
	...
	for (unsigned int i = 0; i < arity; i++)
		if (i != x)
			sum += scope_var [i]->Get_Domain()->Get_Remaining_Real_Value (0);
...
// applies the event-based propagator of the constraint by considering the events occurred since ref
{
  ...
  if (x == arity)
  {
  ...
  else
    {
      ...
      while ((v != -1) && (sum + dx->Get_Real_Value(v) < constant))
        v = dx->Next_Value(v);
        
      if ((v != -1) && (sum + dx->Get_Real_Value(v) == constant))
      {
        ds->Add_Element (scope_var[x]);
        dx->Filter_Value (v);
