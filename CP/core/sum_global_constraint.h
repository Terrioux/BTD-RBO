...
{
	protected:
		long constant;						///< the constant to which the sum is compared
	
	public:
		// constructors
		Sum_Global_Constraint (vector<Variable *> & var, long cst);		///< constructs a new constraint which involves the variable in var and whose relation compares the sum of the values of the variables in var to the constant cst with respect to comp
    ...
    virtual bool Compare (long sum) = 0;           ///< returns true if the sum satisfies the constraint, false otherwise
		long Get_Constant ();											  	///< returns the constant to which the sum is compared
...
inline long Sum_Global_Constraint::Get_Constant ()
